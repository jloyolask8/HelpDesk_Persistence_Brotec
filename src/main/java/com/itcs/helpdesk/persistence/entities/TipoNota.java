...
@NamedQueries({
    ...
    @NamedQuery(name = "TipoNota.findAllByQuery", query = "SELECT t FROM TipoNota t WHERE (LOWER(t.nombre) LIKE CONCAT('%', LOWER(:q), '%')) or (LOWER(t.descripcion) LIKE CONCAT('%', LOWER(:q), '%'))"),
